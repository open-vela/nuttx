...
#ifdef CONFIG_ARCH_CHIP_N947
/* Glitch detector (GDET) registers.
 *
 * On MCXN947 the analog (aGDET) and digital (dGDET) glitch detectors trigger
 * a CHIP_RESET when they observe a supply transient.  Changing the core
 * DCDC/LDO voltage (done in nxxx_clockconfig() below) looks like such a
 * transient, so the detectors must be disabled first.  Otherwise the part
 * resets in an endless boot loop the moment it tries to raise the core
 * voltage.  This mirrors the NXP SDK SystemInit() sequence.
 */

#  define NXXX_ITRC0_OUT_SEL_4_0         (NXXX_ITRC0_BASE + 0x28)
#  define NXXX_ITRC0_OUT_SEL_4_1         (NXXX_ITRC0_BASE + 0x2c)
#  define ITRC_OUT_SEL_IN0_SELN_MASK     (0x3)        /* dGDET selector */
#  define ITRC_OUT_SEL_IN0_SELN_DISABLE  (0x2)
#  define ITRC_OUT_SEL_IN9_SELN_MASK     (0xc0000)    /* aGDET selector */
#  define ITRC_OUT_SEL_IN9_SELN_DISABLE  (0x2 << 18)

#  define NXXX_GDET0_ENABLE1             (NXXX_GDET0_BASE + 0x8)
#  define NXXX_GDET1_ENABLE1             (NXXX_GDET1_BASE + 0x8)
#endif
...
{
#ifdef CONFIG_ARCH_CHIP_N947
  /* Disable the glitch detectors BEFORE nxxx_clockconfig() changes the core
   * voltage, otherwise the voltage transient is flagged as a glitch attack
   * and triggers a CHIP_RESET (endless boot loop).  This mirrors NXP
   * SystemInit().
   */

  /* Route aGDET (ITRC IN9) away from the CHIP_RESET output */

  modifyreg32(NXXX_ITRC0_OUT_SEL_4_0, ITRC_OUT_SEL_IN9_SELN_MASK,
              ITRC_OUT_SEL_IN9_SELN_DISABLE);
  modifyreg32(NXXX_ITRC0_OUT_SEL_4_1, ITRC_OUT_SEL_IN9_SELN_MASK,
              ITRC_OUT_SEL_IN9_SELN_DISABLE);

  /* Disable aGDET interrupt and reset via the SPC */

  modifyreg32(NXXX_SPC_ACTIVE_CFG, 0,
              SPC_ACTIVE_CFG_GLITCH_DETECT_DISABLE_MASK);
  modifyreg32(NXXX_SPC_GLITCH_DETECT_SC, SPC_GLITCH_DETECT_SC_LOCK_MASK, 0);
  putreg32(0x3c, NXXX_SPC_GLITCH_DETECT_SC);
  modifyreg32(NXXX_SPC_GLITCH_DETECT_SC, 0, SPC_GLITCH_DETECT_SC_LOCK_MASK);

  /* Route dGDET (ITRC IN0) away from CHIP_RESET and disable both detectors */

  modifyreg32(NXXX_ITRC0_OUT_SEL_4_0, ITRC_OUT_SEL_IN0_SELN_MASK,
              ITRC_OUT_SEL_IN0_SELN_DISABLE);
  modifyreg32(NXXX_ITRC0_OUT_SEL_4_1, ITRC_OUT_SEL_IN0_SELN_MASK,
              ITRC_OUT_SEL_IN0_SELN_DISABLE);
  putreg32(0, NXXX_GDET0_ENABLE1);
  putreg32(0, NXXX_GDET1_ENABLE1);
#endif
