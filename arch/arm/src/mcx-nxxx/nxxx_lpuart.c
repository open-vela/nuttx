...
{
  int dmaresidual = n947_dmach_getcount(priv->rxdma);
  ...
  if (priv->txch != 0)
    {
      if (priv->txdma == NULL)
        {
          priv->txdma = n947_dmach_alloc(priv->txch, 0);
          if (priv->txdma == NULL)
            {
              return -EBUSY;
            }
  ...
  if (priv->rxch != 0)
    {
      if (priv->rxdma == NULL)
        {
          priv->rxdma = n947_dmach_alloc(priv->rxch, 0);
      ...
      n947_dmach_xfrsetup(priv->rxdma , &config);
      ...
      n947_dmach_start(priv->rxdma, nxxx_dma_rxcallback, (void *)priv);
    }
#endif
...
static void nxxx_dma_txavailable(struct uart_dev_s *dev)
{
  ...
  if (n947_dmach_idle(priv->txdma) == 0)
    {
...
static void nxxx_dma_send(struct uart_dev_s *dev)
{
  ...
  n947_dmach_xfrsetup(priv->txdma, &config);
  ...
  n947_dmach_start(priv->txdma, nxxx_dma_txcallback, (void *)priv);
}
#endif
