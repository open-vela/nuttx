                                            ...
                                            uint16_t *ccmd, uint32_t ncmd)
{
  ...
  return n947_dmach_xfrsetup(priv->txdma, &config);
}
#endif
                                         ...
                                         struct i2c_msg_s *msg)
{
  ...
  return n947_dmach_xfrsetup(dma, &config) ? 0 : msg->length;
}
#endif
...
static int nxxx_lpi2c_dma_transfer(struct nxxx_lpi2c_priv_s *priv)
{
  ...
  n947_dmach_start(priv->rxdma, nxxx_rxdma_callback, (void *)priv);
  n947_dmach_start(priv->txdma, nxxx_txdma_callback, (void *)priv);
  ...
  if ((volatile int)priv->refs++ == 0)
    {
      ...
      if (priv->config->dma_txreqsrc != 0)
        {
          priv->txdma = n947_dmach_alloc(priv->config->dma_txreqsrc, 0);
          DEBUGASSERT(priv->txdma != NULL);
        }

      if (priv->config->dma_rxreqsrc != 0)
        {
          priv->rxdma = n947_dmach_alloc(priv->config->dma_rxreqsrc, 0);
