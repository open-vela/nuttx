/****************************************************************************
 * arch/arm/src/t113/t113_ehci.c
 *
 * T113 USB1 EHCI host driver.
 * Based on arch/arm/src/sama5/sam_ehci.c (Apache 2.0, NuttX upstream).
 *
...
#ifndef CONFIG_T113_EHCI_NQHS
#  define CONFIG_T113_EHCI_NQHS (T113_EHCI_NRHPORT + 1)
#endif
...
#ifndef CONFIG_T113_EHCI_NQTDS
#  define CONFIG_T113_EHCI_NQTDS (T113_EHCI_NRHPORT + 3)
#endif

/* Buffers must be aligned to the cache line size */

#define DCACHE_LINEMASK (ARCH_DCACHE_LINESIZE -1)

/* Configurable size of a request/descriptor buffers */

#ifndef CONFIG_T113_EHCI_BUFSIZE
#  define CONFIG_T113_EHCI_BUFSIZE 128
#endif

#define T113_EHCI_BUFSIZE \
  ((CONFIG_T113_EHCI_BUFSIZE + DCACHE_LINEMASK) & ~DCACHE_LINEMASK)

/* Debug options */

#ifndef CONFIG_DEBUG_USB_INFO
#  undef CONFIG_T113_EHCI_REGDEBUG
#endif
...
/* T113 EHCI register base addresses and host controller pointers ***********/

/* T113 has a single USB1 EHCI port.  The EHCI capability registers start
 * at T113_USB1_EHCI_BASE; the operational registers follow at offset 0x10
 * (the EHCI spec-standard CAPLENGTH value).
 */

#define T113_EHCI_NRHPORT 1

#define HCCR   ((struct ehci_hccr_s *)T113_USB1_EHCI_BASE)
#define HCOR   ((volatile struct ehci_hcor_s *)(T113_USB1_EHCI_BASE + 0x10))
#define HDEBUG ((volatile struct ehci_debug_s *)(T113_USB1_EHCI_BASE + 0x90))

/* T113 uses a flat (physical == virtual) memory map, so phys/virt
 * address translation is a no-op.
 */

#define t113_physramaddr(a) ((uintptr_t)(a))
#define t113_virtramaddr(a) ((uintptr_t)(a))
...
struct t113_qh_s
{
  ...
  struct t113_epinfo_s *epinfo; /* Endpoint used for the transfer */
...
struct t113_list_s
{
  struct t113_list_s *flink;    /* Link to next entry in the list */
                                /* Variable length entry data follows */
};

/* List traversal callout functions */

typedef int (*foreach_qh_t)(struct t113_qh_s *qh, uint32_t **bp, void *arg);
typedef int (*foreach_qtd_t)(struct t113_qtd_s *qtd, uint32_t **bp,
                             void *arg);
...
struct t113_rhport_s
{
  ...
  struct t113_epinfo_s ep0;    /* EP0 endpoint info */
...
struct t113_ehci_s
{
  ...
  struct t113_epinfo_s ep0;     /* Endpoint 0 */
  struct t113_list_s *qhfree;   /* List of free Queue Head (QH) structures */
  struct t113_list_s *qtdfree;  /* List of free Queue Element Transfer Descriptor (qTD) */
  ...
  struct t113_rhport_s rhport[T113_EHCI_NRHPORT];
};
...
static uint16_t t113_read16(const uint8_t *addr);
static uint32_t t113_read32(const uint8_t *addr);
#if 0 /* Not used */
static void t113_write16(uint16_t memval, uint8_t *addr);
static void t113_write32(uint32_t memval, uint8_t *addr);
#endif

#ifdef CONFIG_ENDIAN_BIG
static uint16_t t113_swap16(uint16_t value);
static uint32_t t113_swap32(uint32_t value);
#else
#  define t113_swap16(value) (value)
#  define t113_swap32(value) (value)
#endif

#ifdef CONFIG_T113_EHCI_REGDEBUG
static void t113_printreg(volatile uint32_t *regaddr, uint32_t regval,
         bool iswrite);
static void t113_checkreg(volatile uint32_t *regaddr, uint32_t regval,
         bool iswrite);
static uint32_t t113_getreg(volatile uint32_t *regaddr);
static void t113_putreg(uint32_t regval, volatile uint32_t *regaddr);
#else
static inline uint32_t t113_getreg(volatile uint32_t *regaddr);
static inline void t113_putreg(uint32_t regval, volatile uint32_t *regaddr);
#endif
...
static struct t113_qh_s *t113_qh_alloc(void);
static void t113_qh_free(struct t113_qh_s *qh);
static struct t113_qtd_s *t113_qtd_alloc(void);
static void t113_qtd_free(struct t113_qtd_s *qtd);

/* List Management **********************************************************/

static int t113_qh_foreach(struct t113_qh_s *qh, uint32_t **bp,
         foreach_qh_t handler, void *arg);
static int t113_qtd_foreach(struct t113_qh_s *qh, foreach_qtd_t handler,
         void *arg);
static int t113_qtd_discard(struct t113_qtd_s *qtd, uint32_t **bp,
                            void *arg);
static int t113_qh_discard(struct t113_qh_s *qh);

/* Cache Operations *********************************************************/

#if 0 /* Not used */
static int t113_qtd_invalidate(struct t113_qtd_s *qtd, uint32_t **bp,
         void *arg);
static int t113_qh_invalidate(struct t113_qh_s *qh);
#endif
static int t113_qtd_flush(struct t113_qtd_s *qtd, uint32_t **bp, void *arg);
static int t113_qh_flush(struct t113_qh_s *qh);

/* Endpoint Transfer Handling ***********************************************/

#ifdef CONFIG_T113_EHCI_REGDEBUG
static void t113_qtd_print(struct t113_qtd_s *qtd);
static void t113_qh_print(struct t113_qh_s *qh);
static int t113_qtd_dump(struct t113_qtd_s *qtd, uint32_t **bp, void *arg);
static int t113_qh_dump(struct t113_qh_s *qh, uint32_t **bp, void *arg);
#else
#  define t113_qtd_print(qtd)
#  define t113_qh_print(qh)
#  define t113_qtd_dump(qtd, bp, arg)
#  define t113_qh_dump(qh, bp, arg)
#endif

static inline uint8_t t113_ehci_speed(uint8_t usbspeed);
static int t113_ioc_setup(struct t113_rhport_s *rhport,
         struct t113_epinfo_s *epinfo);
static int t113_ioc_wait(struct t113_epinfo_s *epinfo);
static void t113_qh_enqueue(struct t113_qh_s *qhead, struct t113_qh_s *qh);
static struct t113_qh_s *t113_qh_create(struct t113_rhport_s *rhport,
         struct t113_epinfo_s *epinfo);
static int t113_qtd_addbpl(struct t113_qtd_s *qtd, const void *buffer,
         size_t buflen);
static struct t113_qtd_s *t113_qtd_setupphase(struct t113_epinfo_s *epinfo,
         const struct usb_ctrlreq_s *req);
static struct t113_qtd_s *t113_qtd_dataphase(struct t113_epinfo_s *epinfo,
         void *buffer, int buflen, uint32_t tokenbits);
static struct t113_qtd_s *t113_qtd_statusphase(uint32_t tokenbits);
static int t113_async_setup(struct t113_rhport_s *rhport,
         struct t113_epinfo_s *epinfo, const struct usb_ctrlreq_s *req,
         uint8_t *buffer, size_t buflen);
#ifndef CONFIG_USBHOST_INT_DISABLE
static int t113_intr_setup(struct t113_rhport_s *rhport,
         struct t113_epinfo_s *epinfo, uint8_t *buffer, size_t buflen);
#endif
static ssize_t t113_transfer_wait(struct t113_epinfo_s *epinfo);
#ifdef CONFIG_USBHOST_ASYNCH
static inline int t113_ioc_async_setup(struct t113_rhport_s *rhport,
         struct t113_epinfo_s *epinfo, usbhost_asynch_t callback,
         void *arg);
static void t113_asynch_completion(struct t113_epinfo_s *epinfo);
#endif

/* Interrupt Handling *******************************************************/

static int t113_qtd_ioccheck(struct t113_qtd_s *qtd, uint32_t **bp,
                             void *arg);
static int t113_qh_ioccheck(struct t113_qh_s *qh, uint32_t **bp, void *arg);
static int t113_qtd_cancel(struct t113_qtd_s *qtd, uint32_t **bp, void *arg);
static int t113_qh_cancel(struct t113_qh_s *qh, uint32_t **bp, void *arg);
static inline void t113_ioc_bottomhalf(void);
static inline void t113_portsc_bottomhalf(void);
static inline void t113_syserr_bottomhalf(void);
static inline void t113_async_advance_bottomhalf(void);
static void t113_ehci_bottomhalf(void *arg);
static int t113_ehci_tophalf(int irq, void *context, void *arg);

/* USB Host Controller Operations *******************************************/

static int t113_wait(struct usbhost_connection_s *conn,
         struct usbhost_hubport_s **hport);
static int t113_rh_enumerate(struct usbhost_connection_s *conn,
         struct usbhost_hubport_s *hport);
static int t113_enumerate(struct usbhost_connection_s *conn,
         struct usbhost_hubport_s *hport);
...
static int t113_ioalloc(struct usbhost_driver_s *drvr,
         uint8_t **buffer, size_t buflen);
static int t113_iofree(struct usbhost_driver_s *drvr,
         uint8_t *buffer);
...
static ssize_t t113_transfer(struct usbhost_driver_s *drvr,
...
static int t113_asynch(struct usbhost_driver_s *drvr, usbhost_ep_t ep,
         ...
         void *arg);
#endif
static int t113_cancel(struct usbhost_driver_s *drvr, usbhost_ep_t ep);
#ifdef CONFIG_USBHOST_HUB
static int t113_connect(struct usbhost_driver_s *drvr,
...
static struct t113_ehci_s g_ehci =
{
...
static struct t113_qh_s g_asynchead aligned_data(32);

#ifndef CONFIG_USBHOST_INT_DISABLE
/* The head of the periodic queue */

static struct t113_qh_s g_intrhead   aligned_data(32);

/* The frame list */

#ifdef CONFIG_T113_EHCI_PREALLOCATE
...
static struct t113_qh_s g_qhpool[CONFIG_T113_EHCI_NQHS]
                       aligned_data(32);

/* Queue Element Transfer Descriptor (qTD) pool */

static struct t113_qtd_s g_qtdpool[CONFIG_T113_EHCI_NQTDS]
                        aligned_data(32);
...
static struct t113_qh_s *g_qhpool;

/* Queue Element Transfer Descriptor (qTD) pool */

static struct t113_qtd_s *g_qtdpool;
...
static inline uint32_t t113_read32(const uint8_t *addr)
{
#ifdef CONFIG_ENDIAN_BIG
  return (uint32_t)t113_read16(&addr[0]) << 16 |
         (uint32_t)t113_read16(&addr[2]);
#else
  return (uint32_t)t113_read16(&addr[2]) << 16 |
         (uint32_t)t113_read16(&addr[0]);
#endif
}
...
{
#ifdef CONFIG_ENDIAN_BIG
  t113_write16(memval >> 16, &addr[0]);
  t113_write16(memval & 0xffff, &addr[2]);
#else
  t113_write16(memval & 0xffff, &addr[0]);
  t113_write16(memval >> 16, &addr[2]);
#endif
}
#endif
...
static uint32_t t113_swap32(uint32_t value)
{
  return (uint32_t)t113_swap16((uint16_t)((value >> 16) & 0xffff)) |
         (uint32_t)t113_swap16((uint16_t)(value & 0xffff)) << 16;
}
#endif
                         ...
                         bool iswrite)
{
  ...
  if (regaddr == prevaddr && regval == preval && prevwrite == iswrite)
  ...
  else
    {
      ...
      t113_printreg(regaddr, regval, iswrite);
    }
}
#endif
...
static uint32_t t113_getreg(volatile uint32_t *regaddr)
{
  ...
  t113_checkreg(regaddr, regval, false);
  return regval;
}
#else
static inline uint32_t t113_getreg(volatile uint32_t *regaddr)
...
static void t113_putreg(uint32_t regval, volatile uint32_t *regaddr)
{
  ...
  t113_checkreg(regaddr, regval, true);
  ...
  do
    {
      ...
      regval = t113_getreg(&HCOR->usbsts);
...
static struct t113_qh_s *t113_qh_alloc(void)
{
  ...
  qh = (struct t113_qh_s *)g_ehci.qhfree;
  if (qh)
    {
      g_ehci.qhfree = ((struct t113_list_s *)qh)->flink;
      memset(qh, 0, sizeof(struct t113_qh_s));
    }
...
static void t113_qh_free(struct t113_qh_s *qh)
{
  struct t113_list_s *entry = (struct t113_list_s *)qh;
...
static struct t113_qtd_s *t113_qtd_alloc(void)
{
  struct t113_qtd_s *qtd;

  /* Remove the qTD from the freelist */

  qtd = (struct t113_qtd_s *)g_ehci.qtdfree;
  if (qtd)
    {
      g_ehci.qtdfree = ((struct t113_list_s *)qtd)->flink;
      memset(qtd, 0, sizeof(struct t113_qtd_s));
    }
...
static void t113_qtd_free(struct t113_qtd_s *qtd)
{
  struct t113_list_s *entry = (struct t113_list_s *)qtd;
  ...
  while (qh)
    {
      ...
      physaddr = t113_swap32(qh->hw.hlp);
      ...
      else if (t113_virtramaddr(physaddr & QH_HLP_MASK) ==
       ...
       */
        ...
        {
          physaddr = t113_swap32(qh->hw.hlp) & QH_HLP_MASK;
          next     = (struct t113_qh_s *)t113_virtramaddr(physaddr);
        }
                           ...
                           void *arg)
{
  ...
  physaddr = t113_swap32(*bp); /* Physical address of first qTD in CPU order */
  ...
  qtd  = (struct t113_qtd_s *)t113_virtramaddr(physaddr);
  next = NULL;
  ...
  while (qtd)
    {
      ...
      if ((t113_swap32(qtd->hw.nqp) & QTD_NQP_T) != 0)
        {
          /* Set the next pointer to NULL.  This will terminate the loop. */
        ...
        {
          physaddr = t113_swap32(qtd->hw.nqp) & QTD_NQP_NTEP_MASK;
          next     = (struct t113_qtd_s *)t113_virtramaddr(physaddr);
        }
...
/****************************************************************************
 * Name: t113_qtd_discard
 *
 * Description:
 *   This is a t113_qtd_foreach callback.  It simply unlinks the QTD, updates
 *   the back pointer, and frees the QTD structure.
 *
 ****************************************************************************/

static int t113_qtd_discard(struct t113_qtd_s *qtd, uint32_t **bp, void *arg)
{
...
static int t113_qh_discard(struct t113_qh_s *qh)
{
  ...
  ret = t113_qtd_foreach(qh, t113_qtd_discard, NULL);
  if (ret < 0)
    {
...
/****************************************************************************
 * Name: t113_qtd_invalidate
 *
 * Description:
 *   This is a callback from t113_qtd_foreach.  It simply invalidates D-cache
...
static int t113_qh_invalidate(struct t113_qh_s *qh)
{
  ...
  return t113_qtd_foreach(qh, t113_qtd_invalidate, NULL);
}
#endif
 ...
 *
 * Description:
 *   This is a callback from t113_qtd_foreach.  It simply flushes D-cache for
...
static int t113_qh_flush(struct t113_qh_s *qh)
{
  ...
  return t113_qtd_foreach(qh, t113_qtd_flush, NULL);
}
 ...
 *
 * Description:
 *   This is a t113_qtd_foreach callout function.  It dumps the context
 *   of one qTD
 *
 ****************************************************************************/
 ...
 *
 * Description:
 *   This is a t113_qh_foreach callout function.  It dumps a QH structure and
...
{
  t113_qh_print(qh);
  return t113_qtd_foreach(qh, t113_qtd_dump, NULL);
}
#endif

/****************************************************************************
 * Name: t113_ehci_speed
 *
 * Description:
...
static void t113_qh_enqueue(struct t113_qh_s *qhead, struct t113_qh_s *qh)
{
  ...
  physaddr = (uintptr_t)t113_physramaddr((uintptr_t)qh);
  qhead->hw.hlp = t113_swap32(physaddr | QH_HLP_TYP_QH);
                                      ...
                                      struct t113_epinfo_s *epinfo)
{
  ...
  regval = ((uint32_t)epinfo->devaddr << QH_EPCHAR_DEVADDR_SHIFT) |
           ...
           ((uint32_t)t113_ehci_speed(epinfo->speed) <<
            QH_EPCHAR_EPS_SHIFT) |
           QH_EPCHAR_DTC |
  ...
  qh->hw.epchar = t113_swap32(regval);
...
#warning REVISIT
  ...
  qh->hw.epcaps = t113_swap32(regval);

  /* Mark this as the end of this list.  This will be overwritten if/when the
   * next qTD is added to the queue.
   */

  qh->hw.hlp         = t113_swap32(QH_HLP_T);
  qh->hw.overlay.nqp = t113_swap32(QH_NQP_T);
  qh->hw.overlay.alt = t113_swap32(QH_AQP_T);
  return qh;
}

/****************************************************************************
 * Name: t113_qtd_addbpl
 *
 * Description:
...
/* Maximum bytes per qTD.
 *
 * A qTD has five buffer pointer entries (bpl[0..4]).  Only bpl[0] may have
 * a non-zero 4KB offset; bpl[1..4] must be 4KB-aligned.  Worst case (buffer
 * starts near the end of a 4KB page) one qTD covers just over 4 * 4KB.
 * Pick 16 KiB as a conservative ceiling that always fits regardless of
 * buffer alignment; larger transfers are split across chained qTDs by the
 * caller (see t113_async_setup).
 */

#define T113_QTD_MAX_XFER (4 * 4096)
                          ...
                          size_t buflen)
{
  ...
  physaddr = (uint32_t)t113_physramaddr((uintptr_t)buffer);
  ...
  for (ndx = 0; ndx < 5; ndx++)
    {
      ...
      qtd->hw.bpl[ndx] = t113_swap32(physaddr);
                                            ...
                                            const struct usb_ctrlreq_s *req)
{
  ...
  qtd->hw.nqp = t113_swap32(QTD_NQP_T);
  qtd->hw.alt = t113_swap32(QTD_AQP_T);
  ...
  qtd->hw.token = t113_swap32(regval);

  /* Add the buffer data */

  ret = t113_qtd_addbpl(qtd, req, USB_SIZEOF_CTRLREQ);
  if (ret < 0)
    {
                                           ...
                                           uint32_t tokenbits)
{
  ...
  ret = t113_qtd_addbpl(qtd, buffer, buflen);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_ADDBPL_FAILED, -ret);
      t113_qtd_free(qtd);
                           ...
                           uint8_t *buffer, size_t buflen)
{
  ...
  qh = t113_qh_create(rhport, epinfo);
  if (qh == NULL)
    {
  ...
  if (req != NULL)
    {
      ...
      qtd = t113_qtd_setupphase(epinfo, req);
      if (qtd == NULL)
        {
      ...
      physaddr = t113_physramaddr((uintptr_t)qtd);
      *flink = t113_swap32(physaddr);
  ...
  if (buffer != NULL && buflen > 0)
    {
      uint32_t tokenbits_base;
      uint8_t *p      = (uint8_t *)buffer;
      size_t   remain = buflen;

      /* Compute base tokenbits once.  IOC handling:
       *   - control (req != NULL): data qTDs never set IOC (STATUS qTD
       *     carries IOC).
       *   - non-control (bulk/interrupt): IOC must fire exactly once at
       *     the end of the full transfer, so we set it only on the LAST
       *     chained qTD below.
       */

      tokenbits_base = toggle;

      if (req)
        {
          if ((req->type & USB_REQ_DIR_MASK) == USB_REQ_DIR_IN)
            {
              tokenbits_base |= QTD_TOKEN_PID_IN;
              dirin = true;
            }
          else
            {
              tokenbits_base |= QTD_TOKEN_PID_OUT;
              dirin = false;
            }
        }
      else if (epinfo->dirin)
        {
          tokenbits_base |= (QTD_TOKEN_PID_IN | QTD_TOKEN_IOC);
          dirin = true;
        }
      else
        {
          tokenbits_base |= (QTD_TOKEN_PID_OUT | QTD_TOKEN_IOC);
          dirin = false;
        }

      /* Split the data phase into chunks that each fit in a single qTD
       * (max 5 * 4KB BPL entries ~ 16 KiB worst-case aligned).  Link the
       * chunks together as a chain hanging off *flink.  Only the last
       * chunk keeps its IOC bit so the interrupt fires once.
       */

      while (remain > 0)
        {
          size_t chunk = remain > T113_QTD_MAX_XFER ?
                           T113_QTD_MAX_XFER : remain;
          uint32_t tokenbits = tokenbits_base;

          if (chunk < remain)
            {
              /* Not the final qTD in the chain - suppress IOC. */

              tokenbits &= ~QTD_TOKEN_IOC;
            }

          qtd = t113_qtd_dataphase(epinfo, p, chunk, tokenbits);
          if (qtd == NULL)
            {
          ...
          *flink = t113_swap32(physaddr);
          ...
          /* Advance forward link to this new qTD */

          flink = &qtd->hw.nqp;

          /* For control IN transfers, save the first data qTD's alt
           * pointer so a short packet jumps directly to STATUS.  Chained
           * control IN does not happen in practice (EP0 data is tiny),
           * so tracking only the first qTD matches the legacy single-qTD
           * semantics.
           */
  ...
  if (req != NULL)
    {
      ...
      qtd = t113_qtd_statusphase(tokenbits);
      if (qtd == NULL)
        {
      ...
      *flink = t113_swap32(physaddr);
      ...
      if (alt)
        {
          *alt = t113_swap32(physaddr);
        }
    }

  /* Add the new QH to the head of the asynchronous queue list */

  t113_qh_enqueue(&g_asynchead, qh);
  return OK;
                          ...
                          uint8_t *buffer, size_t buflen)
{
  ...
  qtd = t113_qtd_dataphase(epinfo, buffer, buflen, tokenbits);
  if (qtd == NULL)
    {
      usbhost_trace1(EHCI_TRACE1_QTDDATA_FAILED, 0);
      ret = -ENOMEM;
  ...
  physaddr = t113_physramaddr((uintptr_t)qtd);
  qh->hw.overlay.nqp = t113_swap32(physaddr);

  /* Disable the periodic schedule */

  regval  = t113_getreg(&HCOR->usbcmd);
  regval &= ~EHCI_USBCMD_PSEN;
  t113_putreg(regval, &HCOR->usbcmd);

  /* Add the new QH to the head of the interrupt transfer list */

  t113_qh_enqueue(&g_intrhead, qh);
...
#warning REVISIT
  ...
  /* Did t113_ioc_wait() or nxrmutex_lock report an error? */
...
/****************************************************************************
 * Name: t113_qtd_ioccheck
 *
 * Description:
 *   This function is a t113_qtd_foreach() callback function.  It services
...
static int t113_qtd_ioccheck(struct t113_qtd_s *qtd, uint32_t **bp,
                             void *arg)
{
  struct t113_epinfo_s *epinfo = (struct t113_epinfo_s *)arg;
  ...
  **bp = qtd->hw.nqp;
  ...
  epinfo->xfrd -= (t113_swap32(qtd->hw.token) & QTD_TOKEN_NBYTES_MASK) >>
...
/****************************************************************************
 * Name: t113_qh_ioccheck
 *
 * Description:
 *   This function is a t113_qh_foreach() callback function.  It services one
...
static int t113_qh_ioccheck(struct t113_qh_s *qh, uint32_t **bp, void *arg)
{
  ...
  token = t113_swap32(qh->hw.overlay.token);
  ...
  ret = t113_qtd_foreach(qh, t113_qtd_ioccheck, (void *)qh->epinfo);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
  ...
  if ((t113_swap32(qh->fqp) & QTD_NQP_T) != 0)
    {
...
/****************************************************************************
 * Name: t113_qtd_cancel
 *
 * Description:
 *   This function is a t113_qtd_foreach() callback function.  It removes
 ...
 *
 * Description:
 *   This function is a t113_qh_foreach() callback function.  It cancels
...
static int t113_qh_cancel(struct t113_qh_s *qh, uint32_t **bp, void *arg)
{
  ...
  regval = t113_getreg(&HCOR->usbcmd);
  t113_putreg(regval & ~(EHCI_USBCMD_ASEN | EHCI_USBCMD_PSEN),
               &HCOR->usbcmd);
  ...
  **bp = qh->hw.hlp;
  ...
  ret = t113_qtd_foreach(qh, t113_qtd_cancel, NULL);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
...
static inline void t113_ioc_bottomhalf(void)
{
  ...
  qh = (struct t113_qh_s *)t113_virtramaddr(t113_swap32(*bp) & QH_HLP_MASK);
  ...
  if (qh && qh != &g_asynchead)
    {
      ...
      ret = t113_qh_foreach(qh, &bp, t113_qh_ioccheck, NULL);
      if (ret < 0)
        {
...
static inline void t113_portsc_bottomhalf(void)
{
  ...
  for (rhpndx = 0; rhpndx < T113_EHCI_NRHPORT; rhpndx++)
    {
      rhport = &g_ehci.rhport[rhpndx];
      portsc = t113_getreg(&HCOR->portsc[rhpndx]);
      ...
      t113_putreg(portsc, &HCOR->portsc[rhpndx]);
    }
}

/****************************************************************************
 * Name: t113_syserr_bottomhalf
 *
 * Description:
...
/****************************************************************************
 * Name: t113_async_advance_bottomhalf
 *
 * Description:
...
static void t113_ehci_bottomhalf(void *arg)
{
  ...
  t113_putreg(EHCI_HANDLED_INTS, &HCOR->usbintr);
}

/****************************************************************************
 * Name: t113_ehci_tophalf
 *
 * Description:
...
static int t113_ehci_tophalf(int irq, void *context, void *arg)
{
  ...
  usbsts = t113_getreg(&HCOR->usbsts);
  regval = t113_getreg(&HCOR->usbintr);
  ...
  if (pending != 0)
    {
      ...
      DEBUGVERIFY(work_queue(HPWORK, &g_ehci.work, t113_ehci_bottomhalf,
                            (void *)pending, 0));
                            ...
                            struct usbhost_hubport_s *hport)
{
  ...
  DEBUGASSERT(rhpndx >= 0 && rhpndx < T113_EHCI_NRHPORT);
  rhport = &g_ehci.rhport[rhpndx];
  ...
  regval = t113_getreg(&HCOR->portsc[rhpndx]);
  ...
  DEBUGASSERT((t113_getreg(&HCOR->usbsts) & EHCI_USBSTS_HALTED) == 0);
  ...
  regval  = t113_getreg(regaddr);
  regval &= ~EHCI_PORTSC_PE;
  regval |= EHCI_PORTSC_RESET;
  t113_putreg(regval, regaddr);
  ...
  while ((t113_getreg(regaddr) & EHCI_PORTSC_RESET) != 0);
  ...
  if (ret < 0)
    {
       ...
       * will cause t113_wait() to return and we will try the connection
       * again.
       */
...
{
  struct t113_epinfo_s *epinfo = (struct t113_epinfo_s *)ep0;
  int ret;
                       ...
                       usbhost_ep_t *ep)
{
  ...
  epinfo = kmm_zalloc(sizeof(struct t113_epinfo_s));
  if (!epinfo)
    {
   ...
   * inside of struct t113_epinfo_s and just memcpy here.
   */
...
static int t113_epfree(struct usbhost_driver_s *drvr, usbhost_ep_t ep)
{
  struct t113_epinfo_s *epinfo = (struct t113_epinfo_s *)ep;
                     ...
                     uint8_t **buffer, size_t *maxlen)
{
  ...
  *buffer = (uint8_t *)
    kmm_memalign(ARCH_DCACHE_LINESIZE, T113_EHCI_BUFSIZE);
  if (*buffer)
    {
      *maxlen = T113_EHCI_BUFSIZE;
      ret = OK;
    }
                      ...
                      uint8_t *buffer)
{
  ...
  struct t113_epinfo_s *ep0info = (struct t113_epinfo_s *)ep0;
  uint16_t len;
  ssize_t nbytes;
  int ret;
  ...
  ret = t113_ioc_setup(rhport, ep0info);
  if (ret != OK)
    {
  ...
  ret = t113_async_setup(rhport, ep0info, req, buffer, len);
  if (ret < 0)
    {
      uerr("ERROR: t113_async_setup failed: %d\n", ret);
      goto errout_with_iocwait;
    }

  /* And wait for the transfer to complete */

  nbytes = t113_transfer_wait(ep0info);
  nxrmutex_unlock(&g_ehci.lock);
...
{
  /* t113_ctrlin can handle both directions.  We just need to work around the
   * differences in the function signatures.
   */

  return t113_ctrlin(drvr, ep0, req, (uint8_t *)buffer);
}
                            ...
                            size_t buflen)
{
  ...
  ret = t113_ioc_setup(rhport, epinfo);
  if (ret != OK)
    {
      usbhost_trace1(EHCI_TRACE1_DEVDISCONNECTED, -ret);
      goto errout_with_lock;
  ...
  switch (epinfo->xfrtype)
    {
      case USB_EP_ATTR_XFER_BULK:
        ret = t113_async_setup(rhport, epinfo, NULL, buffer, buflen);
        break;

#ifndef CONFIG_USBHOST_INT_DISABLE
      case USB_EP_ATTR_XFER_INT:
        ret = t113_intr_setup(rhport, epinfo, buffer, buflen);
        break;
#endif
...
# warning "Isochronous endpoint support not emplemented"
  ...
  nbytes = t113_transfer_wait(epinfo);
  nxrmutex_unlock(&g_ehci.lock);
  return nbytes;
                      ...
                      usbhost_asynch_t callback, void *arg)
{
  ...
  ret = t113_ioc_async_setup(rhport, epinfo, callback, arg);
  if (ret != OK)
    {
      usbhost_trace1(EHCI_TRACE1_DEVDISCONNECTED, -ret);
      goto errout_with_lock;
      ...
      case USB_EP_ATTR_XFER_BULK:
        {
          ...
          bp = (uint32_t *)&g_asynchead.hw.hlp;
          qh = (struct t113_qh_s *)
               t113_virtramaddr(t113_swap32(*bp) & QH_HLP_MASK);
...
# warning "Isochronous endpoint support not emplemented"
  ...
  ret = t113_qh_foreach(qh, &bp, t113_qh_cancel, epinfo);
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_QTDFOREACH_FAILED, -ret);
    }
...
struct usbhost_connection_s *t113_ehci_initialize(int controller)
{
  struct usbhost_hubport_s *hport;
  uint32_t regval;
  ...
  DEBUGASSERT((sizeof(struct t113_qh_s) & 0x1f) == 0);
  DEBUGASSERT((sizeof(struct t113_qtd_s) & 0x1f) == 0);
  ...
  /* T113 Hardware Configuration ********************************************/

  /* PHY power-on, CCU clocks, and AHB burst config are handled by
   * t113_usbhost_common_init() which is called once from
   * board_usbhost_initialize() before this function.  Nothing to do here.
   */
  ...
  for (i = 0; i < T113_EHCI_NRHPORT; i++)
    {
      struct t113_rhport_s *rhport = &g_ehci.rhport[i];
      ...
      rhport->drvr.transfer       = t113_transfer;
#ifdef CONFIG_USBHOST_ASYNCH
      rhport->drvr.asynch         = t113_asynch;
#endif
      rhport->drvr.cancel         = t113_cancel;
#ifdef CONFIG_USBHOST_HUB
      rhport->drvr.connect        = t113_connect;
#endif
...
#ifndef CONFIG_T113_EHCI_PREALLOCATE
  /* Allocate a pool of free Queue Head (QH) structures */

  g_qhpool = (struct t113_qh_s *)
    kmm_memalign(32, CONFIG_T113_EHCI_NQHS * sizeof(struct t113_qh_s));
  if (!g_qhpool)
    {
  ...
  for (i = 0; i < CONFIG_T113_EHCI_NQHS; i++)
    {
      /* Put the QH structure in a free list */

      t113_qh_free(&g_qhpool[i]);
    }

#ifndef CONFIG_T113_EHCI_PREALLOCATE
  /* Allocate a pool of free  Transfer Descriptor (qTD) structures */

  g_qtdpool = (struct t113_qtd_s *)
    kmm_memalign(32, CONFIG_T113_EHCI_NQTDS * sizeof(struct t113_qtd_s));
  if (!g_qtdpool)
    {
...
#if !defined(CONFIG_T113_EHCI_PREALLOCATE) && !defined(CONFIG_USBHOST_INT_DISABLE)
  ...
  for (i = 0; i < CONFIG_T113_EHCI_NQTDS; i++)
    {
      /* Put the TD in a free list */

      t113_qtd_free(&g_qtdpool[i]);
    }
  ...
  memset(&g_asynchead, 0, sizeof(struct t113_qh_s));
  physaddr                     = t113_physramaddr((uintptr_t)&g_asynchead);
  g_asynchead.hw.hlp           = t113_swap32(physaddr | QH_HLP_TYP_QH);
  g_asynchead.hw.epchar        = t113_swap32(QH_EPCHAR_H |
                                            QH_EPCHAR_EPS_FULL);
  g_asynchead.hw.overlay.nqp   = t113_swap32(QH_NQP_T);
  g_asynchead.hw.overlay.alt   = t113_swap32(QH_NQP_T);
  g_asynchead.hw.overlay.token = t113_swap32(QH_TOKEN_HALTED);
  g_asynchead.fqp              = t113_swap32(QTD_NQP_T);
  ...
  t113_putreg(t113_swap32(physaddr), &HCOR->asynclistaddr);
  ...
  memset(&g_intrhead, 0, sizeof(struct t113_qh_s));
  g_intrhead.hw.hlp           = t113_swap32(QH_HLP_T);
  g_intrhead.hw.overlay.nqp   = t113_swap32(QH_NQP_T);
  g_intrhead.hw.overlay.alt   = t113_swap32(QH_NQP_T);
  g_intrhead.hw.overlay.token = t113_swap32(QH_TOKEN_HALTED);
  g_intrhead.hw.epcaps        = t113_swap32(QH_EPCAPS_SSMASK(1));

  /* Attach the periodic QH to Period Frame List */

  physaddr = t113_physramaddr((uintptr_t)&g_intrhead);
  for (i = 0; i < FRAME_LIST_SIZE; i++)
    {
      g_framelist[i] = t113_swap32(physaddr) | PFL_TYP_QH;
    }
  ...
  physaddr = t113_physramaddr((uintptr_t)g_framelist);
  t113_putreg(t113_swap32(physaddr), &HCOR->periodiclistbase);
#endif
...
#  endif
  ...
  regval  = t113_getreg(&HCOR->configflag);
  regval |= EHCI_CONFIGFLAG;
  t113_putreg(regval, &HCOR->configflag);
  ...
  if (ret < 0)
    {
      usbhost_trace1(EHCI_TRACE1_RUN_FAILED, t113_getreg(&HCOR->usbsts));
      return NULL;
    }

  /* Interrupt Configuration ************************************************/

  /* Attach EHCI interrupt handler.  T113 uses a dedicated EHCI IRQ (65)
   * separate from the OHCI IRQ (66); no shared dispatcher is needed.
   */

  ret = irq_attach(T113_IRQ_USB1_EHCI, t113_ehci_tophalf, NULL);
  if (ret != 0)
    {
      usbhost_trace1(EHCI_TRACE1_IRQATTACH_FAILED, T113_IRQ_USB1_EHCI);
      return NULL;
    }
  ...
  /* VBUS power: T113 has a single host port.  VBUS is driven externally by
   * the board.  The EHCI PORTSC PP bit is set by the HC when the EHCI
   * config-flag write completes above.  No explicit software action needed.
   */
  ...
  for (i = 0; i < T113_EHCI_NRHPORT; i++)
    {
      g_ehci.rhport[i].connected =
        ((t113_getreg(&HCOR->portsc[i]) & EHCI_PORTSC_CCS) != 0);
    }

  /* Enable interrupts at the interrupt controller */

  up_enable_irq(T113_IRQ_USB1_EHCI); /* enable USB interrupt */
  usbhost_vtrace1(EHCI_VTRACE1_INIITIALIZED, 0);

  return &g_ehciconn;
}

#endif /* CONFIG_T113_EHCI */
