...
#if SOC_INT_CLIC_SUPPORTED
  /* CLIC places interrupt metadata above the cause field.  Keep only the
   * architectural interrupt bit and the cause before using it as an IRQ
   * index.
   */

  mcause &= RISCV_IRQ_BIT | UINT32_C(0x00000fff);
#endif
...
#if SOC_INT_CLIC_SUPPORTED
      /* The P4 CLIC HAL's type-query register is not usable by this
       * NuttX interrupt port.  All sources registered during bring-up are
       * level-triggered, so there is no edge latch to acknowledge here.
       */
