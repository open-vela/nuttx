...
/****************************************************************************
 * MIPI-DSI display controller driver for ESP32-P4.
 *
 * Brings up the full display pipeline of the SoC:
 *
 *   DPI framebuffer -> DW-GDMA channel -> DSI bridge FIFO ->
 *   DSI host (video mode) -> DSI PHY -> EK79007 panel (1024x600)
 *
 * The driver owns the panel power sequence (PHY LDO, panel reset and
 * backlight GPIOs), sends the panel DCS initialization in LP mode,
 * programs the DPI video timing and manages two framebuffers in PSRAM
 * with an explicit front-buffer flip. esp_mipi_dsi_start_refresh()
 * enables the video stream and fills the visible buffer with red as a
 * power-on self test of the whole chain.
 ****************************************************************************/
...
/* Base address of the DSI bridge FIFO window. Address ranges that
 * compare equal to MIPI_DSI_BRG_MEM_BASE are routed to the bridge
 * FIFO by the GDMA master port selector instead of to DRAM, which is
 * how framebuffer data reaches the panel. Must stay in sync with the
 * value used by the vendor HAL. */

#define MIPI_DSI_BRG_MEM_BASE  0x50105000

/****************************************************************************
 * Panel reset and backlight GPIOs
 *
 * On this board the LCD reset line is wired to GPIO27 and the
 * backlight enable to GPIO26, both in the low GPIO register bank
 * (GPIO0-31). The pins are driven through the HP GPIO registers
 * directly: the IO MUX must select the GPIO function (MCU_SEL=1 in
 * bits [14:12] of the per-pin mux register), the output enable is
 * set via the W1TS/W1TC write-one registers and the level via the
 * output W1TS (set) / W1TC (clear) registers.
 ****************************************************************************/
