...
#if defined(CONFIG_FS_ROMFS) && defined(CONFIG_ESP32P4_FUNCTION_EV_AUDIO)
/****************************************************************************
 * Name: funev_signs_romfs_mount
 *
 * Description:
 *   Create a ROM disk from the embedded sign-language voice image and
 *   mount it read-only at /media/signs.
 *
 ****************************************************************************/

#define SIGNS_ROMFS_SECTORSIZE  64
#define SIGNS_ROMFS_NSECTORS(n) \
  (((n) + SIGNS_ROMFS_SECTORSIZE - 1) / SIGNS_ROMFS_SECTORSIZE)
#define SIGNS_ROMFS_MINOR       8

static int funev_signs_romfs_mount(void)
{
  char devname[16];
  int ret;

  ret = romdisk_register(SIGNS_ROMFS_MINOR, g_signs_romfs,
                         SIGNS_ROMFS_NSECTORS(g_signs_romfs_len),
                         SIGNS_ROMFS_SECTORSIZE);
  if (ret < 0)
    {
      syslog(LOG_ERR, "signs romfs: romdisk_register failed: %d\n", ret);
      return ret;
    }

  snprintf(devname, sizeof(devname), "/dev/ram%d", SIGNS_ROMFS_MINOR);

  mkdir("/media", 0777);

  ret = nx_mount(devname, "/media/signs", "romfs", MS_RDONLY, NULL);
  if (ret < 0)
    {
      syslog(LOG_ERR, "signs romfs: mount failed: %d\n", ret);
      return ret;
    }

  syslog(LOG_INFO, "signs romfs: mounted /media/signs (%u bytes)\n",
...
#if defined(CONFIG_ESPRESSIF_I2S) && !defined(CONFIG_ESP32P4_FUNCTION_EV_AUDIO)
  /* Configure I2S peripheral interfaces (generic path).
   * Skipped when the ES8311 codec path is enabled, which registers its
   * own /dev/audio/pcm0 with proper codec configuration.
   */
...
#if defined(CONFIG_VIDEO_FB) && defined(CONFIG_ESP32P4_FUNCTION_EV_LCD)
  /* Register the framebuffer (/dev/fb0).  This triggers up_fbinitialize()
   * which brings up the MIPI-DSI host + EK79007 panel and backlight.
   */
...
#ifdef CONFIG_ESP32P4_FUNCTION_EV_AUDIO
  ret = esp32p4_es8311_initialize(0, FUNEV_ES8311_I2C_ADDR,
                                  FUNEV_ES8311_I2C_FREQ, 0);
  if (ret < 0)
    {
